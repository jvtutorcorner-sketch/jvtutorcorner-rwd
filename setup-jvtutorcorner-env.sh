...
optimize_xrdp_config() {
  ...
  ok "xrdp.ini 優化完成（max_bpp=16、bulk_compression=yes、tcp_keepalive=yes）"
...
optimize_sesman_config() {
  ...
  echo -e "  ${BLUE}→${RESET} 修復 sesman.ini（防止斷線後 session 被殺死）..."
  ...
  ok "sesman.ini 已設定（KillDisconnected=false、IdleTimeLimit=0、DisconnectedTimeLimit=0、MaxSessions=50）"
}
