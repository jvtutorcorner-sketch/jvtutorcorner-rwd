...
export default function AiChatPage() {
    const [messages, setMessages] = useState<Message[]>([
        { id: '1', role: 'assistant', content: '您好！我是您的 AI 助理，有什麼可以幫您的嗎？' }
    ]);
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            setMessages((prev) => [
                ...prev,
                { id: uuidv4(), role: 'assistant', content: '抱歉，系統目前有些忙碌，或尚未設定 AI API Key，請稍後再試。' }
            ]);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-80px)] max-h-screen relative p-4 lg:p-8 bg-gray-50">
            ...
            <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-200 mt-4 flex flex-col overflow-hidden">
                {/* Header */}
                <div className="bg-gradient-to-r from-indigo-600 to-purple-700 text-white p-4 flex justify-between items-center shadow-md shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-semibold leading-tight text-white tracking-wide">AI 聊天室</h3>
                            <p className="text-xs text-indigo-200 mt-0.5">Powered by AI · 智能問答 · 即時回覆</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-4 bg-white border-t border-gray-200 shrink-0">
                    <div className="flex gap-3 relative group max-w-4xl mx-auto">
                        <textarea
                            ...
                            className="flex-1 px-5 py-3.5 bg-gray-50 border border-gray-300 hover:border-gray-400 focus:bg-white rounded-xl focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 text-[15px] transition-all resize-none overflow-hidden"
                            rows={1}
                        ...
                        <button
                            ...
                            className="w-14 h-14 shrink-0 flex items-center justify-center bg-indigo-600 text-white rounded-xl disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed hover:bg-indigo-700 hover:shadow-md transition-all active:scale-95"
                        >
                    ...
                    <div className="text-center mt-3">
                        <span className="text-[11px] text-gray-400 font-medium tracking-wide">💬 Powered by AI · 回應內容僅供參考</span>
                    </div>
                </div>
            </div>
        </div>
