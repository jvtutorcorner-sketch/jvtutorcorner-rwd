...
export default function TeacherCoursesPage() {
  ...
  function validateForm() {
    ...
    if (!form.membershipPlan) { e.membershipPlan = '請選擇會員方案'; }
    setErrors(e);
  ...
  async function handleEdit(course: Course) {
    ...
    if (Object.keys(updates).length === 0) { alert('沒有變更，已取消。'); return; }
    try {
                    ...
                    return (
                      <tr data-course-id={c.id} key={c.id}>
                        ...
                        <td style={{ border: '2px solid #e5e7eb', padding: 8, textAlign: 'center' }}>
                          <a href={`/teacher_courses/${encodeURIComponent(c.id)}/edit`} style={{ marginRight: 8, padding: '6px 10px', display: 'inline-block', textDecoration: 'none', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 4 }}>編輯</a>
