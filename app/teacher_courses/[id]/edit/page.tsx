...
export default function EditCoursePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (!courseId) { setError('找不到課程'); return; }
    const updates: any = {};
    ...
    <div style={{ padding: 16 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 8, maxWidth: 720 }}>
        ...
        <div>
          ...
          <button type="button" onClick={() => router.push('/teacher_courses')} style={{ marginLeft: 8, padding: '8px 12px' }}>取消</button>
        </div>
      </form>
    </div>
  );
