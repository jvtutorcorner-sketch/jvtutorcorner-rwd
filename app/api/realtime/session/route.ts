...
import { generateIceServers, sfuApi, SfuApiError } from '@/lib/realtime/sfuApi';
...
export const POST = withAuth(async (req: AuthedRequest) => {
  ...
  try {
    ...
    const iceServers = await generateIceServers(cfg, decision.tokenExpiresAt - Math.floor(Date.now() / 1000));
