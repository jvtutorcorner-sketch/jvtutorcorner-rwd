 ...
 * picks it up. Fire-and-forget: never block or fail class completion on the summary.
 * Idempotent (markClassEnded only transitions from RECORDING). Gated by CLASS_SUMMARY_ENABLED.
 */
function triggerClassSummary(courseId: string, order: CompletionOrder): void {
  if (process.env.CLASS_SUMMARY_ENABLED !== 'true') return;
  const startMs = parseOrderTime((order as { startTime?: string }).startTime ?? null);
  if (startMs === null) return;
  const summaryId = buildSummaryId(courseId, order.orderId, startMs);
  void markClassEnded(summaryId).catch((e) => console.warn('[classroom/complete] markClassEnded failed', e));
}
