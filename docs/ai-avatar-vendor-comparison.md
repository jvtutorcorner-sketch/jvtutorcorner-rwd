...
| 成本結構 | $0.10/分鐘對話 ＋ RTC頻道費 ＋ Avatar provider費，三筆帳單疊加（有 300 分鐘免費試用） | 依模型按秒/按次計費，60秒影片約 $0.5–3 美元/次，試作抓 $20–40 美元 | GPU租用約 $0.69/hr（RTX 4090），單次算力 $0.1–0.3 美元，但工程與維運人力是主要隱藏成本 |
...
2. **選配音（TTS）模型** — 把腳本文字轉成語音檔，如 `minimax/speech-02`、`xtts-v2` 等模型
3. **選虛擬人臉／口型同步模型** — 大頭照＋語音檔 → 合成影片，如 `SadTalker`、`LivePortrait`、`MuseTalk`
