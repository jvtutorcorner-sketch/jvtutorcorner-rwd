...
> 2026 首頁改版：Hero 右側改為純 CSS 的「線上教室」示意圖（`.cm`，取代原本的 DynamoDB 輪播圖）。
> 未登入主 CTA 改為「探索課程」＋「成為老師」。

#### 3.1 未登入狀態
- [ ] **桌面版**: 顯示「找到適合你的老師，開始你的學習之旅」，版面寬敞
- [ ] **手機版**: 同樣文本正確換行，無溢出
- [ ] CTA 按鈕：「探索課程」、「成為老師」
- [ ] **手機版按鈕**: 堆疊排列，每個 ≥ 40px 高，易於點擊
- [ ] **桌面版按鈕**: 水平排列，充分間距
- [ ] 點擊「探索課程」導向 `/courses`
- [ ] 點擊「成為老師」導向 `/login/register?role=teacher`（老師身分已預選）
...
### 6. Hero 視覺驗證（線上教室示意 `.cm`）

> 改版後 Hero 右側是純 CSS 的線上教室示意（`.home-hero-premium .cm`），不再是輪播圖。

#### 6.1 響應式寬度
- [ ] **桌面版**: `.cm` 寬度不超過視窗，與文字並排
- [ ] **手機版**: `.cm` 不溢出視窗，置於文字與 CTA 下方
- [ ] 含視訊格、白板筆跡、教材頁籤與「上課中」標記
- [ ] 動畫在 `prefers-reduced-motion` 下停用
