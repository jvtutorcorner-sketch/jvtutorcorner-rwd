...
test('完整課程流程：學生與老師分別開啟網頁，同步進入教室', async ({ browser }) => {
    ...
    try {
        ...
        try {
            ...
            const enrollRes = await cleanContext.request.get(`${baseUrl}/api/enroll`);
            const enrollData = await enrollRes.json();
            if (enrollData?.ok && enrollData.data) {
                const relatedEnrollments = enrollData.data.filter((e: any) => e.courseId === testCourseId);
                for (const enr of relatedEnrollments) {
                    await cleanContext.request.delete(`${baseUrl}/api/enroll?id=${enr.id}`);
                }
            }

            // 3. 刪除相關訂單 (Orders)
            console.log(`=== 清理：刪除相關訂單 ===`);
