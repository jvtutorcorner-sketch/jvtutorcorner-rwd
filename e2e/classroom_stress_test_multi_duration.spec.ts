 ...
 *
 * Recording output (headless ≥5 groups only):
...
test.describe('[stress-multi-duration] Escalating Concurrent Loads with Variable Course Durations', () => {
  ...
  for (const durationMinutes of TEST_DURATIONS) {
    for (const concurrentLoad of TEST_CONCURRENT_LOADS) {
      ...
      test(testName, async () => {
        const isHeadless = concurrentLoad >= 5;
        const runId = `${testName}-${Date.now()}`;

        console.log(`🤖 Launching browser in ${isHeadless ? 'HEADLESS + 🎬 Recording' : 'HEADED'} mode (concurrent load: ${concurrentLoad})`);
        if (isHeadless) {
        ...
        try {
          ...
          const groupPromises = groupConfigs.map(async (group) => {
            ...
            // Track whether this group passed for trace labelling
            let groupPassed = false;
            // Create recording-aware contexts (video + trace only when headless)
            const rec = await createGroupRecordingSession(browser, group.groupId, runId, isHeadless);
            const teacherPage = rec.teacherPage;
            const studentPage = rec.studentPage;
            ...
            try {
              ...
              // Finalise recording: saves trace + video (no-op when headed)
              await rec.finalise(groupPassed).catch(() => {});
            }
          });
