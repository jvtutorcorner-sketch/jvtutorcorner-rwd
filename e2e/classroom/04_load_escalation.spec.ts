 ...
 *   CONCURRENT_GROUPS=3   → small class  [headed]
 *   CONCURRENT_GROUPS=5   → medium class [headless + recording]
 *   CONCURRENT_GROUPS=10  → large class  [headless + recording]
 *
 ...
 *
 * Recording output (headless only):
 *   D:\playwright-recordings\<runId>\<groupId>\videos\  (.webm)
 *   D:\playwright-recordings\<runId>\<groupId>\traces\  (.zip)
 *   View trace: npx playwright show-trace "<path>.zip"
 */

import { test, expect, chromium } from '@playwright/test';
import {
  createGroupRecordingSession,
...
test.describe(`[load-${GROUP_COUNT}x] Concurrent Load — ${GROUP_COUNT} Groups`, () => {
  ...
  test(`${GROUP_COUNT} concurrent groups: enrollment → classroom → sync`, async () => {
    const isHeadless = GROUP_COUNT >= 5;
    const runId = `load-${GROUP_COUNT}x-${Date.now()}`;

    console.log(`🤖 Launching browser in ${isHeadless ? 'HEADLESS + 🎬 Recording' : 'HEADED'} mode (GROUP_COUNT: ${GROUP_COUNT})`);
    if (isHeadless) {
    ...
    try {
      ...
      for (let i = 0; i < groupConfigs.length; i++) {
        ...
        const g = groupConfigs[i];
        // createGroupRecordingSession: activates video + trace only when isHeadless=true
        const rec = await createGroupRecordingSession(browser, g.groupId, runId, isHeadless);
        sessions.push({
          idx: i,
          teacherPage: rec.teacherPage,
          studentPage: rec.studentPage,
          finalise: rec.finalise,
        });
      }
      ...
      const testPassed = achievedRate >= SUCCESS_THRESHOLD;
      await Promise.race([
        (async () => {
          // Close pages first to terminate Agora RTC / Netless WebSocket connections,
          // then finalise recordings (saves trace zip + webm video per group).
          for (const s of sessions) {
            await s.teacherPage.close().catch(() => {});
            await s.studentPage.close().catch(() => {});
          }
          await Promise.allSettled(
            sessions.map(s => s.finalise(testPassed && results[s.idx].synced))
          );
