...
async function verifyLive() {
  ...
  // The object keys double as setup-db.mjs step keys (scripts/lib/setup-steps.mjs),
  // so every hint below can name a scoped command instead of the all-tables run —
  // which is unsafe while any declared index still has deployed NULL-key writers.
  const allDefs = [
    ...Object.entries(TABLES).map(([stepKey, def]) => ({ stepKey, def, ownedByUs: true })),
    ...Object.entries(REQUIRED_INDEXES_ON_EXISTING_TABLES).map(([stepKey, def]) => ({ stepKey, def, ownedByUs: false })),
  ];

  for (const { stepKey, def, ownedByUs } of allDefs) {
    const fixCmd = `node scripts/setup-db.mjs --only=${stepKey} --dry-run (then without --dry-run; check the NULL-key deploy order first)`;
    ...
    try {
      ...
      if (err instanceof ResourceNotFoundException) {
        fail('live', `${tableName} does not exist — run: ${fixCmd}`);
        continue;
      }
    ...
    for (const index of def.indexes || []) {
      ...
      if (!actual) {
        fail('live', `${tableName} is missing GSI "${index.name}" — run: ${fixCmd}`);
        continue;
      }
      ...
      } else if (actual.IndexStatus && actual.IndexStatus !== 'ACTIVE') {
        // A backfilling index has the right keys but cannot be queried yet
        // ("Cannot read from backfilling global secondary index").
        fail(
          'live',
          `${tableName} GSI "${index.name}" is ${actual.IndexStatus}, not ACTIVE — wait, or re-run: node scripts/setup-db.mjs --only=${stepKey}`
        );
      } else {
