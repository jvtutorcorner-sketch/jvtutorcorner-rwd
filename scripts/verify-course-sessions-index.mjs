...
check('null → 全部 14 步、順序不變', allSel.selected.length === 14 && eq(allSel.selected.map((s) => s.key), STEP_KEYS) && allSel.skipped.length === 0);
const one = selectSteps(['courseSessions'], fake);
check('courseSessions → 1 步、13 步略過', eq(one.selected.map((s) => s.key), ['courseSessions']) && one.skipped.length === 13);
