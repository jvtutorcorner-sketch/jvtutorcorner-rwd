...
/**
 * Classroom join window only (no LiveKit credentials required).
 *
 * getLiveKitConfig() throws when LIVEKIT_URL / API_KEY / SECRET are unset, which is
 * correct for code that talks to LiveKit but wrong for provider-independent code
 * (schedule queries, capacity pre-scaling, the Cloudflare SFU path) that only needs
 * "how early can people join / how long after the end may they stay".
 */
export function readJoinWindowMinutes(): { earlyJoinMinutes: number; graceMinutes: number } {
  return {
...
export function getLiveKitConfig(): LiveKitConfig {
  ...
  const joinWindow = readJoinWindowMinutes();
  cached = {
    url,
    apiKey,
    apiSecret,
    earlyJoinMinutes: joinWindow.earlyJoinMinutes,
    graceMinutes: joinWindow.graceMinutes,
