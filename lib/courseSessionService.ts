...
//   byStatus    (status    HASH, startTime RANGE)  — all courses, by lifecycle state
...
import { isCourseSessionStatus } from '@/lib/types/courseSession';
import { readJoinWindowMinutes } from '@/lib/livekit/config';
import type {
  CourseSession,
  CourseSessionStatus,
  CreateCourseSessionInput,
  SessionStatusQueryOptions,
  UpcomingAndLiveSessions,
...
/**
 * Query every page. With `maxItems`, stop as soon as that many items are in hand
 * (also sends it as the page Limit) and return exactly that many.
 */
async function queryAll(params: any, maxItems?: number): Promise<CourseSession[]> {
  const cap = maxItems !== undefined && Number.isFinite(maxItems) && maxItems > 0 ? Math.floor(maxItems) : undefined;
  ...
  } while (exclusiveStartKey && (cap === undefined || items.length < cap));
...
/**
 * All sessions in one lifecycle state across every course, startTime ascending,
 * optionally bounded by startTime (inclusive). Uses the byStatus GSI.
 *
 * `status` is a DynamoDB reserved word, hence the `#status` name placeholder.
 * An unknown status returns [] without a request.
 */
export async function listSessionsByStatus(
  status: CourseSessionStatus,
  opts?: SessionStatusQueryOptions
): Promise<CourseSession[]> {
  if (!isCourseSessionStatus(status)) return [];
  ...
  if (opts?.from && opts?.to) {
    params.KeyConditionExpression = '#status = :st AND startTime BETWEEN :from AND :to';
    params.ExpressionAttributeValues[':from'] = opts.from;
    params.ExpressionAttributeValues[':to'] = opts.to;
  } else if (opts?.from) {
    params.KeyConditionExpression = '#status = :st AND startTime >= :from';
    params.ExpressionAttributeValues[':from'] = opts.from;
  } else if (opts?.to) {
    params.KeyConditionExpression = '#status = :st AND startTime <= :to';
    params.ExpressionAttributeValues[':to'] = opts.to;
  } else {
    params.KeyConditionExpression = '#status = :st';
  }

  return queryAll(params, opts?.limit);
}

/**
 * Classes running now plus classes about to start — the input for schedule-driven
 * capacity pre-scaling.
 *
 *   upcoming = SCHEDULED with startTime in
 *              [now − lookbackMinutes, now + withinMinutes + earlyJoinMinutes]
 *   live     = every LIVE session
 *
 * `earlyJoinMinutes` extends the window because people may enter a room that many
 * minutes before startTime. `lookbackMinutes` (default 0) also catches classes whose
 * start has passed but that have not gone LIVE yet (e.g. a late teacher); a
 * pre-scaler should pass the grace period there.
 */
export async function listUpcomingAndLiveSessions(opts: {
  withinMinutes: number;
  earlyJoinMinutes?: number;
  lookbackMinutes?: number;
  now?: Date;
}): Promise<UpcomingAndLiveSessions> {
  const minutes = (v: number | undefined, fallback: number) =>
    v !== undefined && Number.isFinite(v) && v >= 0 ? v : fallback;

  const now = opts.now ?? new Date();
  const within = minutes(opts.withinMinutes, 0);
  const early =
    opts.earlyJoinMinutes !== undefined
      ? minutes(opts.earlyJoinMinutes, 0)
      : readJoinWindowMinutes().earlyJoinMinutes;
  const lookback = minutes(opts.lookbackMinutes, 0);

  const from = new Date(now.getTime() - lookback * 60_000).toISOString();
  const to = new Date(now.getTime() + (within + early) * 60_000).toISOString();

  const [upcoming, live] = await Promise.all([
    listSessionsByStatus('SCHEDULED', { from, to }),
    listSessionsByStatus('LIVE'),
  ]);

  return { live, upcoming, window: { from, to } };
}
