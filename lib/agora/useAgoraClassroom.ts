...
export function useAgoraClassroom({ 
  ...
  const joinWithOptions = useCallback(async (opts?: { publishAudio?: boolean; publishVideo?: boolean; audioDeviceId?: string; videoDeviceId?: string; }) => {
    ...
    try {
      ...
      // DISABLED: Canvas whiteboard initialization (conflicts with Agora whiteboard)
      // Now whiteboard is managed separately via AgoraWhiteboard component
      console.log('[useAgoraClassroom] Whiteboard init disabled - using AgoraWhiteboard component instead');
