  ...
  return (
    <Link href={`/courses/${course.id}`} className={`card course-card ${className}`}>
      <div className="course-card-cover" style={coverStyle}>
        ...
        {enrolled > 0 && (
          <span className="course-card-badge">{t('course_enrolled_count').replace('{count}', String(enrolled))}</span>
        )}
      </div>
      ...
      <div className="course-card-body">
        ...
        <div className="course-card-footer">
          {pointCost !== undefined ? (
            <span className="course-card-price">{t('points_amount').replace('{count}', String(pointCost))}</span>
