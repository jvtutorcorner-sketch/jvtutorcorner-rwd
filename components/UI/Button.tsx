"use client";

import React from 'react';

type Variant = 'primary' | 'secondary' | 'ghost' | 'danger' | 'muted' | 'outline';

type Props = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  variant?: Variant;
  size?: 'sm' | 'md' | 'lg';
  className?: string;
};

const variantMap: Record<Variant, string> = {
  primary: 'bg-[#7c3aed] text-white hover:bg-[#6d28d9]',
  secondary: 'bg-[#ede9fe] text-[#6d28d9] hover:bg-[#ddd6fe] border border-[#7c3aed]/20',
  ghost: 'bg-white text-gray-700 hover:bg-gray-50 border border-gray-200',
  danger: 'bg-red-600 text-white hover:bg-red-700',
  muted: 'bg-gray-100 text-gray-700 hover:bg-gray-200',
  outline: 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50',
};

const sizeMap: Record<NonNullable<Props['size']>, string> = {
  sm: 'px-2 py-1 text-sm',
  md: 'px-3 py-1.5 text-sm',
  lg: 'px-4 py-2 text-sm',
};

export default function Button({ 
  variant = 'primary', 
  size = 'md', 
  className = '', 
  disabled, 
  onClick,
  children, 
  ...rest 
}: Props) {
  const base = 'inline-flex items-center justify-center rounded font-medium transition-colors focus:outline-none cursor-pointer';
  const v = variantMap[variant] || variantMap.primary;
  const s = sizeMap[size] || sizeMap.md;
  const disabledCls = disabled ? 'opacity-60 cursor-not-allowed pointer-events-none' : '';
  const cls = [base, v, s, disabledCls, className].filter(Boolean).join(' ');

  // Ensure onClick handler is properly bound
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    if (disabled) {
      e.preventDefault();
      return;
    }
    if (onClick) {
      onClick(e);
    }
  };

  return (
    <button 
      type="button"
      className={cls} 
      disabled={disabled} 
      onClick={handleClick}
      {...rest}
    >
      {children}
    </button>
  );
}
