 ...
 * @deprecated 改用 <T k="..." />。
 *
 * 保留完整的舊簽章（k / fallback / vars / s）——origin/main 那側的頁面
 * （app/courses/[id]、app/teachers/[id]）有 27 處是用 `k=` 呼叫的，只留 `s`
 * 會讓它們全部壞掉。兩種形式都轉交給 <T>。
 */
export function ServerT({ k, fallback, s, vars }: { k?: string; fallback?: string; s?: string; vars?: TVars }) {
  const key = k ?? s;
  if (!key) return null;
  return <T k={key} fallback={fallback} vars={vars} />;
}
