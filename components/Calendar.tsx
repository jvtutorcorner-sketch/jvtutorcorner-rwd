                ...
                {selectedEvent.courseId && (
                  <div className="mb-2">
                    {(() => {
                      const user = getStoredUser();
                      const now = Date.now();
                      const startTs = selectedEvent.start.getTime();
                      const endTs = selectedEvent.end.getTime();
                      
                      // Allow 10 minutes early entry
                      const canEnter = now >= (startTs - 10 * 60 * 1000) && now <= endTs;
                      
                      if (!canEnter) {
                        return (
                          <div className="w-full inline-flex items-center justify-center px-4 py-2 bg-gray-100 text-gray-400 rounded-lg cursor-not-allowed font-medium text-sm border border-gray-200" title="僅在課程開始前 10 分鐘開放進場">
